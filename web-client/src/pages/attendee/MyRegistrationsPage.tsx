...
import { Ticket, Ban, MessageSquareText, CalendarSearch } from 'lucide-react';
          ...
          return (
            <Card key={r.registrationId} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                ...
                <Link to={`/events/${r.eventId}`} className="mt-2 block text-base font-semibold text-ink hover:text-accent">
                  {r.eventTitle}
                </Link>
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-2">
                {!cancelled && (
                  <Button size="sm" onClick={() => onTicket(r)} leftIcon={<Ticket className="h-4 w-4" />}>
                    Ticket
                  </Button>
                )}
                {past ? (
